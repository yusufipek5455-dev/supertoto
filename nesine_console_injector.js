/**
 * ANTIGRAVITY NESINE SPOR TOTO DEVTOOLS CONSOLE INJECTOR (ZERO-INSTALL) v4.5
 * =========================================================================
 * 40 TL Sheets Architecture (1 Kupon = Harf A, B, C, D = 4 Kolon = 40 TL)
 * Core Presets: 960 TL (96 Kolon) & 480 TL (48 Kolon)
 * React / Angular Native Property Setter & Automated Multi-Sheet Bridge
 */

(function() {
    console.log("%c⚡ Antigravity Nesine Spor Toto Multi-Covering Injector v4.5 Başlatılıyor...", "color: #38bdf8; font-weight: bold; font-size: 14px;");

    const existing = document.getElementById('antigravity-nesine-widget');
    ...
    widget.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #334155; padding-bottom: 8px; margin-bottom: 10px;">
            ...
            <div style="cursor: pointer; font-size: 16px; color: #94a3b8;" id="ag-close-btn">&times;</div>
        </div>

        <div id="ag-body">
            <div style="margin-bottom: 8px; color: #94a3b8; font-size: 11px;">
                Streamlit Cockpit'ten kopyaladığınız JSON payload'ı buraya yapıştırın:
            </div>
            <textarea id="ag-payload-input" placeholder="📋 40 TL Sheet JSON verisini buraya yapıştırın..." style="
                width: 100%;
                height: 70px;
                background: #1e293b;
    ...
    document.getElementById('ag-close-btn').onclick = () => widget.remove();

    document.getElementById('ag-btn-parse').onclick = () => {
    ...
    // REACT / ANGULAR SAFE CHECKBOX SETTER
    async function clickNesineBox(el, targetChecked) {
        ...
        if (isChecked !== targetChecked) {
            ...
            // Native property descriptor setter for React 16+ controlled component reconciliation
            try {
    ...
    document.getElementById('ag-btn-clear').onclick = async () => {
    ...
    async function injectFullSheet(sheet) {
        ...
        for (const l of letters) {
            if (sheet[l] && sheet[l].length === 15) {
                ...
                // Explicitly clear unused letter slots to prevent ghost bets
    ...
    document.getElementById('ag-btn-inject-sheet').onclick = async () => {
    ...
    document.getElementById('ag-btn-auto-step').onclick = async () => {
        ...
        // Try triggering "Hemen Oyna" or "Kuponu Kaydet / Sepete Ekle"
    ...
    document.querySelectorAll('.ag-btn-letter').forEach(btn => {
        btn.onclick = async (e) => {
            if (isInjecting || !sheetsList[currentSheetIdx]) return;
    ...
    console.log("%c⚡ Antigravity Multi-Covering 40 TL Injector Arayüzü Ekrana Eklendi!", "color: #4ade80; font-weight: bold;");
})();
